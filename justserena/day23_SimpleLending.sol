...
contract SimpleLending {
    ...
    uint256 public collateralFactorBasisPoints = 7500;//如果你压了价值 100 块钱的玩具，你最多只能借走 75 块钱。
    ...
    function borrow(uint256 amount) external {
        ...
        uint256 maxBorrowAmount = (collateralBalances[msg.sender] * collateralFactorBasisPoints) / 10000;//算出你最多能借多少钱
        uint256 currentDebt = calculateInterestAccrued(msg.sender);//算算你现在一共欠了多少钱（本金+利息）

        require(currentDebt + amount <= maxBorrowAmount, "Exceeds allowed borrow amount");//你原来的欠款 + 这次想借的钱，不能超过你的最高限额
